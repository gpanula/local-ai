...
class PipelineRunCommand(BaseCommand):
    ...
    @staticmethod
    def _format_telemetry(raw_stats: str, role: Optional[str] = None) -> str:
        ...
        return (
            f"*Generated by `{model}`{role_str}: {tokens} tokens in {duration}s ({tps_str}){res_str}"
            f" | {ctx_str}*"
        )
    ...
    def _handle_review(args, review_verdict, prompt_content, final_code_block, reviewer_history,
        ...
        if review_stats:
            transport.send_terminal_mcp(f"📊 {PipelineRunCommand._format_telemetry(review_stats, role='reviewer')}")
